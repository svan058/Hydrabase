...
export const ActivityFeed = ({ eventLog }: Props) => {
  ...
  return (
    <div style={{
      ...
      bottom: 28,           // sits on top of StatusBar
      left: 0,
      position: "fixed",
      right: 0,
      zIndex: 47,
        ...
        <div style={{
          ...
          {[...eventLog].reverse().slice(0, 30).map((entry, i) => (
            <div key={i} style={{
              ...
              <span style={{
                color: LV_COLOR[entry.lv] ?? ACCENT,
                flexShrink: 0,
                fontSize: 9,
                fontWeight: 700,
                letterSpacing: ".06em",
              ...
              <span style={{ color: entry.lv === "ERROR" ? "#ffa198" : entry.lv === "WARN" ? "#e3b341" : TEXT }}>
                {entry.m}
              </span>
            </div>
          ))}
          {eventLog.length === 0 && (
            <div style={{ color: MUTED, fontSize: 11, padding: "8px 14px" }}>No events yet…</div>
          )}
        </div>
      )}
      ...
      >
        ...
        {latest ? (
          <>
            <span style={{ color: LV_COLOR[latest.lv] ?? ACCENT, flexShrink: 0, fontSize: 9, fontWeight: 700, letterSpacing: ".06em" }}>
              {latest.lv}
            </span>
            <span style={{
              color: MUTED,
        ...
        {/* toggle chevron */}
        <span style={{ color: MUTED, flexShrink: 0, fontSize: 10, marginLeft: 6, transform: expanded ? "rotate(180deg)" : "none", transition: "transform .2s" }}>
          ▲
        </span>
      </div>
    </div>
