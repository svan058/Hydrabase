...
interface TableProps {
  ...
  SI: ({ k }: { k: keyof ApiPeer | "icon" }) => JSX.Element
  toggleSort: (k: keyof ApiPeer) => void
}

const PEER_HEADER: [keyof ApiPeer | "icon", string][] = [
  ["icon", ""],
  ["status", "Status"],
  ["username", "Username"],
  ["address", "Address"],
...
const Table = ({ peers, sel, setSel, SI, toggleSort }: TableProps) => <table style={{ borderCollapse: "collapse", width: "100%" }}>
  <thead>
    <tr style={{ background: "#0d1117", color: MUTED, fontSize: 10, textTransform: "uppercase" }}>
      {PEER_HEADER.map(([k, l]) => <th key={String(k)} onClick={() => { if (k !== "icon") toggleSort(k as keyof ApiPeer) }} style={{ cursor: "pointer", fontWeight: 700, letterSpacing: ".07em", padding: "7px 12px", textAlign: "left", whiteSpace: "nowrap" }}>{l} <SI k={k} /></th>)}
    </tr>
  </thead>
  <tbody>
    {peers.map(p => <tr className="rh" key={p.address} onClick={() => setSel(sel?.address === p.address ? null : p)} style={{ background: sel?.address === p.address ? "rgba(88,166,255,.05)" : "transparent", borderTop: `1px solid ${BORD}`, cursor: "pointer" }}>
      <td style={{ padding: "8px 8px 8px 12px", width: 32 }}>
        <Identicon address={p.address} size={22} />
      </td>
...
export const OverviewTab = ({ peers, sel, setSel, SI, toggleSort, votes }: Props) => {
